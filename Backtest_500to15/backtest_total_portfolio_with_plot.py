...
class StockAnalyzer:
    ...
    def get_month_end_dates(self, year: int) -> pd.DataFrame:
        # Since the portfolio is selected at the last date of every month,
        # we also get the last date of previous year and first date of next year
        y1 = year-1
        y2 = year+1
        query = f"""
    ...
    def get_daily_portfolio_value(self, symbols: List[str], start_date: str, end_date: str) -> pd.DataFrame:
        ...
        open_prices = portfolio_value.iloc[0]
        
        # Calculate the adjusted daily portfolio value
        adjusted_portfolio_value = portfolio_value.div(open_prices)
        ...
        individual_stock_returns = [
            (symbol, (last_closes[symbol] - open_prices[symbol]) / open_prices[symbol] * 100) 
    ...
    def get_full_portfolio_series(self, all_monthly_portfolio_values: List[pd.DataFrame], portfolio_size: int) -> pd.Series:
        ...
        full_portfolio_series.to_csv('../Levitas/results/daily_portfolio_returns.csv', index=True)

        # Write symbols by month in file: symbols_by_month.csv
        symbols_by_month.to_csv('../Levitas/results/symbols_by_month.csv', index=True)
        

        return full_portfolio_series

    def plot_portfolio_performance(self, full_portfolio_series: List[pd.Series], start_year: int, end_year: int, max_drawdown: float) -> None:
        """
        ...
        # Save the plot
        plt.savefig(f'../Levitas/results/portfolio_performance_{start_year}_{end_year}.png', dpi=300, bbox_inches='tight')
        plt.show()
        plt.close()    
        return total_return    
    ...
    def analyze_top_stocks(self, year: int, portfolio_size: int) -> Tuple[List[List[str]], List[Tuple[str, float]], float, float]:
        ...
        try:
            ...
            for i in range(months_in_year):
                ...
                portfolio_selection_date_str = month_end_dates.iloc[i]['Last_Date_Of_Month'].strftime('%Y-%m-%d')
                next_portfolio_selection_date_str = month_end_dates.iloc[i + 1]['Last_Date_Of_Month'].strftime('%Y-%m-%d')

                # Step 2: Get top 15 stocks on portfolio_selection_date
                df_portfolio = self.get_top_stocks(portfolio_selection_date_str, portfolio_size)
                ...
                current_portfolio = df_portfolio['NSE_Symbol'].tolist()
    ...
    def write_results(self, year: int, monthly_portfolio: List[List[str]], 
        ...
        filename = f"../Levitas/results/long_15_from_500/results_{year}.txt"
        try:
...
def main():
    ...
    full_portfolio_series = analyzer.get_full_portfolio_series(all_monthly_portfolio_values, portfolio_size)
    # Create single plot for all years
    total_portfolio_return = analyzer.plot_portfolio_performance(full_portfolio_series, years[0], years[-1], min(portfolio_drawdown))
