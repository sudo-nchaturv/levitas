...
class StockAnalyzer:
    ...
    def get_daily_portfolio_value(self, symbols: List[str], start_date: str, end_date: str) -> pd.DataFrame:
        ...
        individual_returns = [(symbol, (last_closes[symbol] - first_closes[symbol]) / first_closes[symbol] * 100) for symbol in symbols]
    
        return adjusted_portfolio_value, individual_returns
    ...
    # def portfolio_plot(portfolio, returns):
    ...
    def analyze_top_stocks(self, year: int, portfolio_size: int) -> Tuple[List[List[str]], List[Tuple[str, float]], float, float]:
        try:
            ...
            for i in range(months_in_year):
                ...
                # Get top 15 stocks on current_date_str
                df_top_15 = self.get_top_stocks(current_date_str, portfolio_size)
                ...
                portfolio_values, individual_returns = self.get_daily_portfolio_value(
                    current_portfolio,
                    current_date_str,
                    next_date_str
                )
                ...
                monthly_individual_returns.append(individual_returns)
            ...
            #self.portfolio_plot(all_portfolio_values, monthly_returns)
            ...
            return monthly_portfolio, monthly_returns, total_return, max_drawdown, monthly_individual_returns
...
def main():
    ...
    for year in years:
        ...
        try:
            ...
            monthly_portfolio, monthly_returns, total_return, max_drawdown, monthly_individual_returns = analyzer.analyze_top_stocks(year, portfolio_size)
            
            total_monthly_portfolio.append(monthly_portfolio)
            total_monthly_returns.append(monthly_returns)

            analyzer.write_results(year, monthly_portfolio, monthly_returns, total_return, max_drawdown, monthly_individual_returns)
    ...
    logger.info(f"Total Return: {np.sum(portfolio_returns):.2f}%")
